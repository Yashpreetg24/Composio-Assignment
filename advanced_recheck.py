...
from runner import load_seed_data, load_existing_results, save_results_json, export_results_csv
...
def extract_domain(hint: str) -> str:
    # Basic cleanup
    hint = hint.split(' ')[0].strip() # e.g. "twenty.com (open-source CRM)" -> "twenty.com"
...
def advanced_research(app_data: dict) -> Any:
    ...
    for url in urls_to_try:
        ...
        if len(text) > 500 and ('API' in text or 'developer' in text.lower() or 'endpoints' in text.lower()):
            best_text = text
            best_url = url
            break
...
def main():
    ...
    for app_data in apps_to_run:
        ...
        try:
            ...
            logger.info(f"Verdict: {old_verdict} -> {new_verdict} | Confidence: {old_conf} -> {new_conf}")
