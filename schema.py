...
class AppResearchResult(BaseModel):
    ...
    auth_methods: List[str] = Field(
        description="Array of auth methods, e.g. ['OAuth2'], ['API key'], ['Basic Auth'], ['Token']"
    )
    self_serve: Literal[
        "self-serve", "gated-paid", "gated-approval", "gated-partnership"
    ] = Field(
    ...
    buildability_verdict: Literal[
        "buildable-now", "buildable-with-friction", "not-buildable"
    ] = Field(description="Verdict on how easily an integration can be built right now")
    verdict_reason: str = Field(
        description="One-line reason/blocker for the buildability verdict"
    )
    evidence_url: str = Field(
        description="The actual docs URL used to determine the information"
    )
    confidence: Literal["high", "medium", "low"] = Field(
        description="How sure the agent is about the extracted information"
    )
    needs_human_review: bool = Field(
        description="True if the agent couldn't find clear info or encountered an error"
    )
